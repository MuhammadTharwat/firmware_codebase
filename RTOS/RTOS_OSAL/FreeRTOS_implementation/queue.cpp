...
namespace rtos_osal
{
  queue::queue(const char *pachr_queue_name, uint32_t u32_depth, size_t sz_entry_size, void *pv_queue_mem)
  {
    ...
    str_queue_mem.hdl_queue = xQueueCreateStatic(u32_depth, sz_entry_size, reinterpret_cast<uint8_t *>(pv_queue_mem), &str_queue_mem.str_q_attr);
  ...
  tenu_osal_status queue::receive(void *pv_buff, size_t sz_buff_len, uint32_t u32_timeout_ms)
  {
    ...
    else
    {
      ...
      enu_status = xQueueReceive(this->str_queue_mem.hdl_queue, pv_buff, (std::numeric_limits<uint32_t>::max() == u32_timeout_ms) ? portMAX_DELAY : (u32_timeout_ms / portTICK_PERIOD_MS));
  ...
  tenu_osal_status queue::send(const void *pv_data, size_t sz_buff_len, uint32_t u32_timeout_ms)
  {
    ...
    {
      BaseType_t enu_status;
      enu_status = xQueueSend(this->str_queue_mem.hdl_queue, pv_data, (std::numeric_limits<uint32_t>::max() == u32_timeout_ms) ? portMAX_DELAY : (u32_timeout_ms / portTICK_PERIOD_MS));
      enu_osal_status = osal_err_map(enu_status);
    }
    return enu_osal_status;
  }
  ...
  {
    return reinterpret_cast<uint32_t>(this->str_queue_mem.hdl_queue);
  }
